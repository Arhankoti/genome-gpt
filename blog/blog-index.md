...
8. 📝 **Feeding the Starving Model** — the Arc I finale: grow the corpus 6→20 genomes, freeze the model and budget, and watch the honest number finally move (it does — biggest on the large models Part 6 showed were starving) while the memorization gap that fanned open in Part 6 stays shut. Doubles as the retrospective: what a six-letter language taught me about *measuring* honestly. *(Part 8 — code landed: `scaling.py --compare` + `align_runs` + `viz.render_scaling_compare` + a `download.py` header fix; post drafted: [`blog-8-feeding-the-starving-model.md`](blog-8-feeding-the-starving-model.md).)*
...
9. 📝 **Scoring, Out Loud** — turning "bits per base" into a plain-English verdict (dna_like / random_like / low_complexity), guarded by a complexity check and a composition-preserving **shuffle test**: real held-out *M. tuberculosis* reads `dna_like` (1.93 bits), its shuffle collapses to `random_like` (2.01). *(Part 9 — code landed: `dna_score_report` tool + `generation.score_verdict`/`composition_shuffle` + `score.py` + `viz.render_score_report`; post drafted: [`blog-9-scoring-out-loud.md`](blog-9-scoring-out-loud.md).)*
